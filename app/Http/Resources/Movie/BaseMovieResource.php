...
class BaseMovieResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        $array = [
            'id' => $this->id,
            'titles' => (new TitleCollection($this->titles))->resolve(),
            ...
            'type' => (int)$this->type,
            'descriptions' => (new DescriptionCollection($this->descriptions))->resolve(),
        ...
        $array['links'] = MinimalLinkResource::collection($this->links()->where('status', '1')->orderBy('language_type_id', 'ASC')->orderBy('created_at', 'DESC')->get())->resolve();
